...
export const tableRelationGroups: TableRelationGroup[] = [
  ...
  },
  {
    ...
    description: "員工首頁 BFF 依目前場館組合櫃台交接、班表、公告、文件與場租查看。",
    Icon: FileText,
    children: [
      { table: "operational_handovers", relation: "facilityKey -> targetDate / targetShiftLabel", feature: "交辦事項", status: "active" },
      { table: "operational_handovers", relation: "facilityKey -> createdBy / assignee", feature: "櫃台交接", status: "active" },
      { table: "employee_resources", relation: "facilityKey + category", feature: "活動檔期、常用文件、本地公告", status: "active" },
...
export const featureFlows: FeatureFlow[] = [
  {
    ...
    owner: "employee",
    route: "/employee",
    summary: "登入後依 activeFacility 取首頁 BFF，再把櫃台交接、班表、公告、文件、場租組成固定格線。",
    steps: ["Auth session", "activeFacility", "employee home BFF", "home cards", "page / drawer / detail routes"],
    tables: ["sessions_index", "operational_handovers", "employee_resources", "system_announcements", "court_reservations"],
  ...
  },
  {
    ...
    route: "/lifeguard/* -> /supervisor/facilities/:facilityKey",
    summary: "救生員先拿 GPS，再拍照疊浮水印，上傳 Object Storage，主管在單館場館狀態頁查看各 lifeguard_* table 與 audit log。",
  ...
  },
  {
    ...
    title: "日誌模板到主管審核流",
    owner: "supervisor / lifeguard",
    route: "/supervisor/facilities/:facilityKey",
    summary: "主管在場館狀態頁檢視救生填報與附件；交辦事項改由 handover 共用資料庫承載。",
...
export const routeRelations: RouteRelation[] = [
  {
    ...
    title: "員工端",
    route: "/employee",
    modules: ["櫃台交接", "今日家教預約 placeholder", "群組重要公告", "今日班表", "活動檔期", "常用文件", "場租查看"],
