...
const NAV: NavSuffix[] = [
  { suffix: "", label: "單日場租", icon: <CalendarDays className="w-4 h-4" />, testId: "nav-day" },
  ...
  { suffix: "/search", label: "搜尋場租", icon: <Search className="w-4 h-4" />, testId: "nav-search" },
  { suffix: "/admin", label: "預約管理", icon: <Settings className="w-4 h-4" />, testId: "nav-admin" },
];

export const getCourtsBasePath = (location: string) =>
  location.startsWith("/employee/courts") ? "/employee/courts" : "/supervisor/courts";
...
export function AppHeader({
  ...
  const switchSchool = (newSchool: SchoolId) => {
    ...
    const m = location.match(/^\/(?:supervisor|employee)\/courts\/[a-z]+(\/[^?]*)?(\?.*)?$/);
    const suffix = m?.[1] ?? "";
    const search = m?.[2] ?? "";
    navigate(`${basePath}/${newSchool}${suffix}${search}`);
  };
      ...
      <div>
        <div className="flex min-h-12 items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <Link href={`${basePath}/${school}`}>
              <div
                className="flex items-center gap-2 cursor-pointer group"
                data-testid="brand-link"
              >
                <div className="w-7 h-7 rounded-[6px] bg-[#102940] flex items-center justify-center transition group-hover:bg-[#2f9e5b]">
                  <Trophy className="w-4 h-4 text-white" />
                </div>
                <h1
                  className="hidden sm:block text-[15px] font-black text-[#102940] tracking-tight whitespace-nowrap"
            ...
            <div
              className="hidden md:flex items-center bg-[#f4f6f8] rounded-[7px] p-0.5"
              ...
              {SCHOOLS.map((s) => (
                <button
                  ...
                  className={`px-2.5 h-7 rounded text-xs font-medium transition ${
                    s.id === school
                      ? "bg-white text-[#102940] shadow-sm"
                      : "text-[#7c8998] hover:text-[#102940]"
                  }`}
                >
                  {s.shortName}
                </button>
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link key={item.to} href={item.to}>
                    ...
                    className={`px-2.5 h-7 rounded-[6px] text-xs font-bold flex items-center gap-1 transition ${
                        active
                          ? "bg-[#102940] text-white"
                          : "text-[#536175] hover:bg-[#f4f6f8] hover:text-[#102940]"
                      }`}
                    >
                      {item.icon}
                      {item.label}
        ...
        <div
          className="flex md:hidden items-center bg-[#f4f6f8] rounded-[7px] p-0.5 mb-2"
            ...
            return (
              <Link key={item.to} href={item.to}>
                <button
                  ...
                  className={`px-2.5 h-7 rounded-[6px] text-xs font-bold flex items-center gap-1 whitespace-nowrap transition ${
                    active
                      ? "bg-[#102940] text-white"
                      : "text-[#536175] hover:bg-[#f4f6f8] hover:text-[#102940]"
                  }`}
