...
專案目前已收斂到 Modular Monolith + Module Registry + BFF 的方向。舊路由仍保留，但新功能應先進 module registry，再接 BFF / API / UI。模組閉合狀態以 `docs/governance/MODULE_CLOSURE_MATRIX.md` 為準；retired 模組不得被當成待完成 active surface。

後續施工以 `docs/planning/PHASE_TOPOLOGY_MAP.md` 為拓樸主圖：先判斷所在 Phase，再動 schema / BFF / API / UI，避免功能只完成單層接線。驗收門檻以 `docs/planning/PHASE_ACCEPTANCE_GATES.md` 為準。施工技能選型以 `docs/operations/SKILL_ASSISTED_CONSTRUCTION_ROUTER.md` 為準，必要時先用 VoltAgent `awesome-agent-skills` 與 `npx skills find` 查技能，再開始改碼。UI/UX 類施工另以 `docs/design/UIUX_SKILL_REVIEW_PROTOCOL.md` 為硬閘門。
...
flowchart LR
  ...
  BFF --> PG["Postgres / employee_resources / handover / telemetry"]
...
已接受為 background / legacy / integration 且未接 BFF 的 registry module：
...
| 個人工作記事 / 便利貼 | retired / deploy-pending | 員工 | active route removed | retired | historical `employee_resources category=sticky_note` | 部署套用 `0014_retire_tasks_personal_note.sql` 後確認資料清空 |
...
| 任務 | retired / deploy-pending | `/supervisor/tasks` 與 `/api/tasks` 已移出 active workbench；主管交辦流改由 handover/work-log/counter-log 承接 | 部署套用 `0014_retire_tasks_personal_note.sql` 後確認 `tasks` table 移除 |
...
| 工作區 dirty 檔案很多 | 推送前需整理 staging，避免截圖/output 一起上傳；retirement、LINE smoke、closure docs 應獨立成可審查批次 |
...
依 `docs/planning/PHASE_TOPOLOGY_MAP.md`，目前先回到 Phase 0 的寫入路徑補強，再進 Phase 1/2。
...
- Phase 0.1c 第二棒：`employee_resources` create/update 使用 metadata helper；`sticky_note` 為歷史資料，現已進入 retirement cleanup。
...
- Phase 0.1c 第五棒：`tasks` 曾完成 create/update/status metadata；現已自 active workbench 退休，保留為歷史紀錄。
...
- Phase 0 T0.1.5：`quick_links`、`employee_resources`、`operational_handovers`、`system_announcements` 與歷史 `tasks update/status` 曾補 `recordAudit()`；`tasks` 現已進入 retirement cleanup。
- Phase 0 T0.1.6：`tasks create` 曾改用 `withTaskCreateMetadata()` 並寫 `TASK_CREATED` audit；現為歷史紀錄，不得作為新施工入口。
...
- Phase 1 UIUX 第一輪：對照 `ui-ux-pro-max` 與 Vercel Web Interface Guidelines，補強 Employee shell/home/documents 與當時的 personal-note 焦點狀態；personal-note 現已退休。
...
- Phase 2 T2.3：參考 `主管端ui.zip` 但不直接套用固定寬度 mock JSX；主管端 shell 改為全 viewport 自適應、左側導覽收斂工作台入口（移除設定）；`/supervisor/tasks` 現已退休。
...
1. Replit DB migration / 實資料 smoke：部署 DB 後跑常用文件、教材觀看、異常收件者新增更新，查 `audit_logs` 對應 row，並確認 `0014_retire_tasks_personal_note.sql` 已移除 `tasks` table / `sticky_note` rows。
