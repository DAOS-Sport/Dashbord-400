...
關鍵驗收與極端情況驗收請看 `docs/planning/PHASE_ACCEPTANCE_GATES.md`。拓樸決定施工順序，驗收門檻決定是否可進下一刀。

施工技能路由請看 `docs/operations/SKILL_ASSISTED_CONSTRUCTION_ROUTER.md`。每一刀先判斷 Phase，再決定要啟用的 skill，最後才動 schema / BFF / API / UI。

所有 UI/UX 類施工必須再對照 `docs/design/UIUX_SKILL_REVIEW_PROTOCOL.md`。員工端不是只求可用，而是要做到足夠好用、好看、穩定，讓員工日常上班自然依賴。
...
2. 對照 `docs/operations/SKILL_ASSISTED_CONSTRUCTION_ROUTER.md`，確認本輪要啟用的 skill。
3. 若涉及 UI/UX，先對照 `docs/design/UIUX_SKILL_REVIEW_PROTOCOL.md`。
...
12. 對照 `docs/planning/PHASE_ACCEPTANCE_GATES.md` 的關鍵與極端情況驗收點。

每次完成後：

1. 更新 `docs/planning/CONSTRUCTION_MAP.md`。
