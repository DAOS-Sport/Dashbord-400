...
| `lifeguard` | Lifeguard shell | `/lifeguard/*`, selected shared `/employee/*` read surfaces | Mobile-first lifeguard operations and log records | Parking, supervisor reports, system raw data |
| `supervisor` | Supervisor workbench | `/supervisor/*` | Operations control, facility status, approvals, parking, courts, lifeguard overview | System raw inspector and registry administration |
...
|---|---|
| `employee` | `employee-home`, `handover`, `activity-periods`, `employee-resources`, `employee-training`, `personal-note`, `lifeguard-lost-and-found`, `courts`, `knowledge-base-qna`, `checkins` |
| `lifeguard` | `lifeguard-home`, `lifeguard-water-quality`, `lifeguard-coach-dive`, `lifeguard-cleanup`, `lifeguard-lane-issues`, `lifeguard-lost-and-found`, `lifeguard-lane-rentals`, `lifeguard-log`, `announcements`, `employee-training`, `knowledge-base-qna` |
| `supervisor` | `supervisor-dashboard`, `facilities`, `parking`, `counter-log`, `lane-rentals`, `courts`, `tasks`, `announcements`, `announcement-groups`, `supervisor-lifeguard-overview`, `handover`, `employee-training`, `anomalies`, `analytics` |
| `system` | `system-dashboard`, `system-function-relations`, `system-topology`, `system-health`, `system-observability`, `integration-sync-jobs`, `telemetry-audit`, `system-lifeguard-audit`, `raw-inspector`, `employee-training` |

## Write Permissions By Domain

| Domain | Employee | Lifeguard | Supervisor | System | Audit Expectation |
|---|---:|---:|---:|---:|---|
| Employee handover | create/update own work surface | shared read where routed | review/manage | observe through audit | `OPERATIONAL_HANDOVER_*`, `HANDOVER_ENTRY_*` |
