---
module_id: system-observability
...
3. 功能 / 需求 / 用途：Legacy observability entry retained as a Watchdog/Governance tab source. 狀態：legacy / 相容層。

## Registry Snapshot

- Module ID: `system-observability`
- Status: legacy / 相容層
- Domain: `system`
...
- 入口從 `/system`、`/system/integrations`、`/system/audit` 進入，依角色 system、SYSTEM_ADMIN 顯示。
- 讀取透過 `GET /api/bff/system/overview`、`GET /api/bff/system/integration-overview`、`GET /api/bff/system/ui-event-overview`。
- 目前沒有登記寫入 API；視為 read-only、external、planned 或 legacy surface。
- 外部或基礎依賴：POSTGRES。
- 資料落點 / entity：`system_overview_projection`、`integration_error_logs`、`bff_latency_logs`。
...
- Telemetry / audit 資料採 append-only 或事件式寫入，避免覆寫歷史：`integration_error_logs`、`bff_latency_logs`。
...
- 畫面資料應優先吃 BFF section / endpoint：`observability`、`/api/bff/system/overview`。
- 目前 read-only 或背景型；若新增互動寫入，先補 registry API、BFF contract、audit/telemetry。
- UI telemetry：page view、audit required。

## BFF 參照 / 修改關聯

- BFF endpoint owner：`GET /api/bff/system/overview`、`GET /api/bff/system/integration-overview`、`GET /api/bff/system/ui-event-overview`。
- Section key / planned endpoint：systemSectionKey=`observability`、plannedEndpoint=`/api/bff/system/overview`。
...
- 資料：確認 `system_overview_projection`、`integration_error_logs`、`bff_latency_logs` 的讀寫方向沒有繞過 owner module。
...
| /system/audit | system | system | implemented |
...
| Binding | Value |
| --- | --- |
| systemSectionKey | observability |
...
| integration_error_logs | integration error | telemetry | partial |  |
| bff_latency_logs | BFF latency | telemetry | planned |  |

## Integrations

| Provider | Purpose | Status | Notes |
| --- | --- | --- | --- |
| POSTGRES | Observability persistence and projections. | partial |  |
...
- Governance notes: Partial until latency and integration error write paths are fully wired.
