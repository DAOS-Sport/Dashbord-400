...
- 畫面資料應優先吃 BFF section / endpoint：`bookingSnapshot`、`/api/bff/employee/home`。
...
| Binding | Value |
| --- | --- |
| employeeSectionKey | bookingSnapshot |
...
| --- | --- | --- | --- |
| UNKNOWN | Booking adapter exists as a boundary but real provider is not connected. | planned |  |
...
- Governance notes: Planned only; connect through booking adapter before exposing as real data.
