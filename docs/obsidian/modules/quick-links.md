...
3. 功能 / 需求 / 用途：Supervisor-managed shortcuts used by portal and employee home. 狀態：implemented / 已接線。

## Registry Snapshot

- Module ID: `quick-links`
...
- 入口從 `/portal/:facilityKey/manage`、`/employee/more` 進入，依角色 employee、lifeguard、supervisor 顯示。
- 讀取透過 `GET /api/portal/quick-links`。
- 寫入透過 `POST /api/portal/quick-links`、`PATCH /api/portal/quick-links/:id`、`DELETE /api/portal/quick-links/:id`。
- 外部或基礎依賴：POSTGRES。
- 資料落點 / entity：`quick_links`。
...
- 寫入 API 需保留權限檢查、審計或狀態切換語意：`POST /api/portal/quick-links`、`PATCH /api/portal/quick-links/:id`、`DELETE /api/portal/quick-links/:id`。
...
- 畫面資料應優先吃 BFF section / endpoint：`shortcuts`。
...
- Section key / planned endpoint：employeeSectionKey=`shortcuts`。
- 寫入後 BFF 需要刷新或重算的 CRUD endpoint：`GET /api/portal/quick-links`、`POST /api/portal/quick-links`、`PATCH /api/portal/quick-links/:id`、`DELETE /api/portal/quick-links/:id`。
...
- 資料：確認 `quick_links` 的讀寫方向沒有繞過 owner module。
...
| --- | --- | --- | --- |
| /portal/:facilityKey/manage | employee | legacy_portal | legacy |
...
| POST | /api/portal/quick-links | crud | implemented |
| PATCH | /api/portal/quick-links/:id | crud | implemented |
| DELETE | /api/portal/quick-links/:id | crud | implemented |

### BFF Sections

| Binding | Value |
| --- | --- |
| employeeSectionKey | shortcuts |
...
| --- | --- | --- | --- | --- |
| quick_links | quick link | postgres | implemented |  |

## Integrations

| Provider | Purpose | Status | Notes |
| --- | --- | --- | --- |
| POSTGRES | Local shortcut persistence. | implemented |  |

## Telemetry / Governance

- Telemetry: pageView=yes；cardClick=yes；actionSubmit=yes；auditRequired=no
- Event types: link_click
- Editable by: supervisor, system
- Readonly for: employee
- Requires approval: no
- Governance notes: Supervisor manages facility-scoped quick links.
