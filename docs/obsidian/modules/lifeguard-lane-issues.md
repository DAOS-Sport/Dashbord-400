---
module_id: lifeguard-lane-issues
label: "水道事項"
status: partial
domain: support
...
3. 功能 / 需求 / 用途：Lifeguard lane notes frame for lane rental, temporary change, and incident notes. 狀態：partial / 部分接線。

## Registry Snapshot

- Module ID: `lifeguard-lane-issues`
- Status: partial / 部分接線
- Domain: `support`
...
- 入口從 `/lifeguard/lane-issues`、`/lifeguard/lane-notes` 進入，依角色 lifeguard 顯示。
- 沒有登記讀取 API；資料多半由其他 projection 或背景流程提供。
- 寫入透過 `POST /api/bff/lifeguard/lane-issues`。
- 外部或基礎依賴：POSTGRES。
- 資料落點 / entity：`lifeguard_handover_notes`。

## 資料寫法 / 寫入規則

- 資料權威：`postgres`。
- Postgres 寫入需通過 server module / storage layer，不應在前端直接寫表：`lifeguard_handover_notes`。
- 沒有 projection 資料登記。
- 沒有 telemetry 資料登記。
- 沒有 external data binding。
- 寫入 API 需保留權限檢查、審計或狀態切換語意：`POST /api/bff/lifeguard/lane-issues`。
...
- BFF endpoint owner：`POST /api/bff/lifeguard/lane-issues`。
...
- 資料：確認 `lifeguard_handover_notes` 的讀寫方向沒有繞過 owner module。
...
| --- | --- | --- | --- |
| /lifeguard/lane-issues | lifeguard | lifeguard | partial |
| /lifeguard/lane-notes | lifeguard | lifeguard | legacy |

## API / BFF

| Method | Path | Kind | Status |
| --- | --- | --- | --- |
| POST | /api/bff/lifeguard/lane-issues | bff | partial |
...
| --- | --- | --- | --- | --- |
| lifeguard_handover_notes | lane notes and handover records | postgres | partial |  |

## Integrations

| Provider | Purpose | Status | Notes |
| --- | --- | --- | --- |
| POSTGRES | Future lane note storage through work-log handover categories. | partial |  |
...
- Governance notes: 水道事項 stays under lifeguard work-log and lane-rentals context.
