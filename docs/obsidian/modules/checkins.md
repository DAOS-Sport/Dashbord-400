...
3. 功能 / 需求 / 用途：Attendance and check-in entry placeholder for future attendance provider integration. 狀態：planned / 預留。
...
- 入口從 `/employee/checkins` 進入，依角色 employee 顯示。
...
- 畫面資料應優先吃 BFF section / endpoint：`checkins`。
...
- Section key / planned endpoint：employeeSectionKey=`checkins`。
...
| --- | --- | --- | --- |
| /employee/checkins | employee | employee | partial |
...
| --- | --- | --- | --- | --- |
| source_snapshots | future check-in source snapshot | external | planned |  |

## Integrations

| Provider | Purpose | Status | Notes |
| --- | --- | --- | --- |
| UNKNOWN | Future attendance or check-in provider. | planned |  |
...
- Governance notes: Not connected; route renders not-connected surface through EmployeeMorePage.
