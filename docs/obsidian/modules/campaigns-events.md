...
3. 功能 / 需求 / 用途：Employee-visible events/campaigns from employee resources and announcement candidate fallback. 狀態：partial / 部分接線。

## Registry Snapshot

- Module ID: `campaigns-events`
- Status: partial / 部分接線
- Domain: `support`
...
- 入口從 `/portal/:facilityKey/campaigns`、`/employee/announcements` 進入，依角色 employee、lifeguard、supervisor 顯示。
- 讀取透過 `GET /api/portal/employee-resources`。
- 寫入透過 `POST /api/portal/employee-resources`。
...
- 寫入 API 需保留權限檢查、審計或狀態切換語意：`POST /api/portal/employee-resources`。
...
- 畫面資料應優先吃 BFF section / endpoint：`campaigns`。
...
- Section key / planned endpoint：employeeSectionKey=`campaigns`。
- 寫入後 BFF 需要刷新或重算的 CRUD endpoint：`GET /api/portal/employee-resources`、`POST /api/portal/employee-resources`。
...
| --- | --- | --- | --- |
| /portal/:facilityKey/campaigns | employee | legacy_portal | legacy |
| /employee/announcements | employee | employee | partial |
...
| --- | --- | --- | --- | --- |
| employee_resources | event resource | postgres | implemented | category=event |

## Integrations

| Provider | Purpose | Status | Notes |
| --- | --- | --- | --- |
| LINE_BOT_ASSISTANT | Campaign fallback extracted from announcement candidates. | external |  |
...
- Governance notes: Employee-created campaign/event content is allowed but scoped by facility.

## Legacy

- Old names: 無
- Old routes: /portal/:facilityKey/campaigns
- Migration notes: 無
