---
module_id: announcement-summary
...
3. 功能 / 需求 / 用途：Announcement dashboard summary and weekly report export view. 狀態：partial / 部分接線。

## Registry Snapshot

- Module ID: `announcement-summary`
- Status: partial / 部分接線
- Domain: `derived`
...
- 入口從 `/announcements/summary` 進入，依角色 supervisor、system 顯示。
- 讀取透過 `GET /api/announcement-dashboard/summary`、`GET /api/announcement-reports/weekly`。
...
- 畫面資料應優先吃 BFF section / endpoint：`announcementSummary`、`/api/announcement-dashboard/summary`。
...
- Section key / planned endpoint：supervisorSectionKey=`announcementSummary`、plannedEndpoint=`/api/announcement-dashboard/summary`。
- 沒有 CRUD endpoint；BFF 可視為 read-only projection 或外部相容層。
- Proxy / external 邊界：`GET /api/announcement-dashboard/summary`、`GET /api/announcement-reports/weekly`；前端不得繞過此邊界。
...
| --- | --- | --- | --- |
| /announcements/summary | system | legacy_admin | implemented |
...
| Binding | Value |
| --- | --- |
| supervisorSectionKey | announcementSummary |
| plannedEndpoint | /api/announcement-dashboard/summary |
...
| --- | --- | --- | --- | --- |
| source_snapshots | announcement summary snapshot | external | partial |  |

## Integrations

| Provider | Purpose | Status | Notes |
| --- | --- | --- | --- |
| LINE_BOT_ASSISTANT | Summary and weekly report upstream. | external |  |
...
- Governance notes: Summary reads upstream report data; local persistence is not complete.

## Legacy

- Old names: 無
- Old routes: /announcements/summary
- Migration notes: 無
