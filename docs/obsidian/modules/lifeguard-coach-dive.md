---
module_id: lifeguard-coach-dive
label: "教練下水"
status: partial
domain: support
...
3. 功能 / 需求 / 用途：Lifeguard coach dive GPS photo records with coach name hook. 狀態：partial / 部分接線。

## Registry Snapshot

- Module ID: `lifeguard-coach-dive`
- Status: partial / 部分接線
- Domain: `support`
...
- 入口從 `/lifeguard/coach-dive`、`/lifeguard/coach-water-photo` 進入，依角色 lifeguard 顯示。
- 沒有登記讀取 API；資料多半由其他 projection 或背景流程提供。
- 寫入透過 `POST /api/bff/lifeguard/coach-dive`、`POST /api/bff/lifeguard/photo-upload`。
- 外部或基礎依賴：OBJECT_STORAGE。
- 資料落點 / entity：`lifeguard_coach_dive_logs`。

## 資料寫法 / 寫入規則

- 資料權威：`postgres`。
- Postgres 寫入需通過 server module / storage layer，不應在前端直接寫表：`lifeguard_coach_dive_logs`。
- 沒有 projection 資料登記。
- 沒有 telemetry 資料登記。
- 沒有 external data binding。
- 寫入 API 需保留權限檢查、審計或狀態切換語意：`POST /api/bff/lifeguard/coach-dive`、`POST /api/bff/lifeguard/photo-upload`。
...
- BFF endpoint owner：`POST /api/bff/lifeguard/coach-dive`、`POST /api/bff/lifeguard/photo-upload`。
...
- 資料：確認 `lifeguard_coach_dive_logs` 的讀寫方向沒有繞過 owner module。
...
| --- | --- | --- | --- |
| /lifeguard/coach-dive | lifeguard | lifeguard | partial |
| /lifeguard/coach-water-photo | lifeguard | lifeguard | legacy |

## API / BFF

| Method | Path | Kind | Status |
| --- | --- | --- | --- |
| POST | /api/bff/lifeguard/coach-dive | bff | partial |
...
| --- | --- | --- | --- | --- |
| lifeguard_coach_dive_logs | coach dive GPS photo records | postgres | partial |  |

## Integrations

| Provider | Purpose | Status | Notes |
| --- | --- | --- | --- |
| OBJECT_STORAGE | Coach dive photo upload storage. | partial |  |
