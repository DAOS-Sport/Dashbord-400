...
1. 角色：employee；可見角色 employee, lifeguard, supervisor
2. RAGIC / 資料庫：不使用 Ragic；資料源為 postgres
3. 功能 / 需求 / 用途：Employee-owned documents, Notion-like links, events, and sticky notes. 狀態：implemented / 已接線。

## Registry Snapshot

- Module ID: `employee-resources`
...
- 入口從 `/employee/documents`、`/employee/more`、`/portal/:facilityKey/manage` 進入，依角色 employee、lifeguard、supervisor 顯示。
...
- 畫面資料應優先吃 BFF section / endpoint：`documents`、`settings`。
...
- Section key / planned endpoint：employeeSectionKey=`documents`、supervisorSectionKey=`settings`。
...
| --- | --- | --- | --- |
| /employee/documents | employee | employee | partial |
...
| --- | --- | --- | --- |
| GET | /api/portal/employee-resources | crud | implemented |
| POST | /api/portal/employee-resources | crud | implemented |
| PATCH | /api/portal/employee-resources/:id | crud | implemented |
| DELETE | /api/portal/employee-resources/:id | crud | implemented |

### BFF Sections

| Binding | Value |
| --- | --- |
| employeeSectionKey | documents |
| supervisorSectionKey | settings |
...
| --- | --- | --- | --- | --- |
| employee_resources | employee resource | postgres | implemented | sub_category and sort_order support document link categorization and user ordering. |

## Integrations

| Provider | Purpose | Status | Notes |
| --- | --- | --- | --- |
| POSTGRES | Employee-created resources. | implemented |  |
...
- Governance notes: Employees can create scoped resources; supervisors can moderate/edit where allowed.
