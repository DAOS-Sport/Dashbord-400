...
3. 功能 / 需求 / 用途：Legacy portal review view for announcement candidate approval. 狀態：partial / 部分接線。

## Registry Snapshot

- Module ID: `portal-review`
- Status: partial / 部分接線
- Domain: `legacy`
...
- 入口從 `/portal/:facilityKey/review` 進入，依角色 supervisor、system 顯示。
- 讀取透過 `GET /api/announcement-candidates`。
...
- 畫面資料應優先吃 BFF section / endpoint：`announcementReview`。
...
- Section key / planned endpoint：supervisorSectionKey=`announcementReview`。
- 沒有 CRUD endpoint；BFF 可視為 read-only projection 或外部相容層。
- Proxy / external 邊界：`GET /api/announcement-candidates`、`POST /api/announcement-candidates/:id/approve`、`POST /api/announcement-candidates/:id/reject`；前端不得繞過此邊界。
...
| --- | --- | --- | --- | --- |
| audit_logs | candidate review audit | postgres | planned |  |

## Integrations

| Provider | Purpose | Status | Notes |
| --- | --- | --- | --- |
| LINE_BOT_ASSISTANT | Announcement candidate review proxy. | external |  |
...
- Governance notes: Legacy portal review should be replaced by supervisor announcement module over time.

## Legacy

- Old names: 無
- Old routes: /portal/:facilityKey/review
- Migration notes: 無
