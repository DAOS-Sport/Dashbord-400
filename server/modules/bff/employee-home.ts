...
export const getEmployeeHomeMock = (): EmployeeHomeDto => ({
  ...
  shifts: stale(
    ...
    syncTime,
    "排班系統正式來源尚未重連，暫以測試 projection 顯示",
  ),
  campaigns: ok(
    [
      {
...
export const getSystemOverviewMock = (): SystemOverviewDto => ({
  ...
  integrationFailures: ok(
    [
      {
        id: "f-1",
        title: "Ragic auth adapter 未接正式來源",
        time: "mock",
        severity: "info",
      },
      {
        id: "f-2",
        title: "Booking adapter 等待 Replit 重連",
        time: "mock",
        severity: "info",
      },
    ],
...
const smartScheduleOverviewUrl = () => {
  const path = env.smartScheduleApiToken
...
export const getSupervisorDashboardFromSources =
  async (): Promise<SupervisorDashboardDto> => {
    ...
    const [announcementSummary, scheduleOverview] = await Promise.allSettled([
      ...
      fetchJson<Record<string, unknown>>(
        smartScheduleOverviewUrl(),
        env.smartScheduleApiToken,
      ),
    ]);

    const announcementData =
      announcementSummary.status === "fulfilled"
        ? announcementSummary.value
        : null;
    const scheduleData =
      scheduleOverview.status === "fulfilled" ? scheduleOverview.value : null;
    const employees =
      scheduleData?.employees && typeof scheduleData.employees === "object"
        ? (scheduleData.employees as Record<string, unknown>)
        : {};
    const shiftsToday =
      scheduleData?.shiftsToday && typeof scheduleData.shiftsToday === "object"
        ? (scheduleData.shiftsToday as Record<string, unknown>)
        : {};
    const pendingReviewCount = Number(
      announcementData?.pendingReviewCount ?? 0,
    );
    const activeUsers = Number(
      scheduleData?.activeUsers ??
        scheduleData?.todayActiveUsers ??
        employees.totalActive ??
        0,
    );
    const totalUsers = Number(
      scheduleData?.totalUsers ?? employees.totalAll ?? activeUsers ?? 0,
    );
    const onShift = Number(
      scheduleData?.onShift ?? shiftsToday.total ?? activeUsers ?? 0,
    );
    ...
    return {
        ...
        ? ok(
            {
              ...
              total: totalUsers || 0,
              onShift: onShift || 0,
              absent: Math.max((totalUsers || 0) - (activeUsers || 0), 0),
            },
            now,
          )
      ...
      announcementAcks: announcementData
        ? ok(
            {
              unconfirmed: pendingReviewCount,
              totalRequired: Number(
                announcementData.totalMessagesToday ?? pendingReviewCount,
              ),
            },
            now,
          )
...
export const getSystemOverviewFromSources =
  async (): Promise<SystemOverviewDto> => {
    const now = new Date().toISOString();
    ...
    const failures = checks
      .map((check, index) => ({
        check,
        label: index === 0 ? "LINE Bot Assistant" : "Smart Schedule Manager",
      }))
    ...
    return {
      checkedAt: now,
      healthScore: ok(
        { score, statusLabel: score === 100 ? "健康" : "部分異常" },
        now,
      ),
      metrics: ok(
        [
          {
            label: "外部 API 健康度",
            value: `${score}%`,
            status: score === 100 ? "ok" : "warning",
            helper: "LINE Bot / Smart Schedule",
          },
          {
            label: "LINE Bot Assistant",
            value: checks[0].status === "fulfilled" ? "正常" : "異常",
            status: checks[0].status === "fulfilled" ? "ok" : "critical",
            helper: env.lineBotBaseUrl,
          },
          {
            label: "Smart Schedule",
            value: checks[1].status === "fulfilled" ? "正常" : "異常",
            status: checks[1].status === "fulfilled" ? "ok" : "critical",
            helper: env.smartScheduleBaseUrl,
          },
          {
            label: "資料庫模式",
            value: env.databaseProfile,
            status: env.databaseProfile === "mock" ? "warning" : "ok",
            helper: "DATABASE_PROFILE",
          },
          {
            label: "資料來源模式",
            value: env.dataSourceMode,
            status: env.dataSourceMode === "mock" ? "warning" : "ok",
            helper: "DATA_SOURCE_MODE",
          },
        ],
        now,
      ),
